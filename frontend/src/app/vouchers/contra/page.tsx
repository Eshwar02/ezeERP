...
export default function ContraPage() {
  ...
  function load() {
    api<Voucher[]>("/accounting/vouchers?type=Contra", { company: true }).then(setRows).catch(() => {});
  }
  useEffect(load, []);

  return (
    <Shell title="Contra Voucher">
      <AccountingVoucherForm voucherType="Contra" onSaved={load} />
      <h2 className="mb-3 mt-8 text-lg font-bold text-slate-700">Recent Contra Entries</h2>
      <div className="card overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {rows.length === 0 && (
              <tr><td className="px-4 py-6 text-center text-slate-400" colSpan={4}>No contra entries yet.</td></tr>
            )}
            {rows.map((v) => (
