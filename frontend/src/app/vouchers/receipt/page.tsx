...
export default function ReceiptPage() {
  ...
  function load() {
    api<Voucher[]>("/accounting/vouchers?type=Receipt", { company: true }).then(setRows).catch(() => {});
  }
  useEffect(load, []);

  return (
    <Shell title="Receipt Voucher (F6)">
      <AccountingVoucherForm voucherType="Receipt" onSaved={load} />
      <h2 className="mb-3 mt-8 text-lg font-bold text-slate-700">Recent Receipts</h2>
      <div className="card overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {rows.length === 0 && (
              <tr><td className="px-4 py-6 text-center text-slate-400" colSpan={4}>No receipts yet.</td></tr>
            )}
            {rows.map((v) => (
